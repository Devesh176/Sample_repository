...
### Path Planning : We have divided the path planning into two parts mainly Global and Local Planner. For Gobal planner we are going with two algoritms RRT* or A* search algorithms based on the map of environment, and for local planner we are going with machine-learning based model-predictive control algorithm. So basically the global planner will give us the set of waypoints to be followed and local planner will be used for connecting two waypoints.
 
 Here will come the pictures of global planner
 and waypoints video
### Computer Vision : 
